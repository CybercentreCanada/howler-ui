...
export default abstract class TuiPhraseLexer {
  private _phrase: string = null;

  private _buffer: string[] = null;

  private _tokens: TuiPhraseToken[] = null;

  private _consumers: TuiPhraseConsumer<TuiPhraseLexer>[] = null;

  private _consumer: TuiPhraseConsumer<TuiPhraseLexer> = null;

  private _startIndex: number = 0;

  private _endIndex: number = -1;

  private find(cursor: number, tokens: TuiPhraseToken[]): { token: TuiPhraseToken; index: number } {
    const index = tokens.findIndex(t => t.startIndex <= cursor && cursor <= t.endIndex);
    const token = tokens.at(index);
    return { index, token };
  }

  public parse(phrase: string, cursor: number = 0): TuiPhraseAnalysis {
    this._phrase = phrase;
    this._buffer = [];
    this._tokens = [];
  ...
  public lock(): TuiPhraseConsumer<TuiPhraseLexer> {
  ...
  abstract consumers(): TuiPhraseConsumer<TuiPhraseLexer>[];
}
