...
import { TuiListItemOnSelect, TuiListMenuRenderer } from '..';
...
type TuiTableProps<T> = {
  ...
  onSort?: (model: TuiSearchTerms) => void;
...
export default function TuiTable<T>({
  keyboard,
  flexPort,
  headless,
  columns,
  ...
  return (
    <TableRoot ref={ref} data-tuitable="true">
      {!headless && layout && <TuiTableHead layout={layout} columns={columns} sorters={sorters} onSort={onSort} />}
      {layout &&
        (flexPort ? (
          <FlexPort>
            <div ref={bodyRef}>
